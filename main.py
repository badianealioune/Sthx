...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")
        await self.highrise.walk_to(Position(11.5 , 0.0 , 1.5 , "FrontLeft"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.chat(f"Bienvenue {user.username} au Lycée GT😈❤️.Pour danser utilise de 1 à 95.Ecris help pour voir la liste des commandes")
      
        await self.highrise.send_emote("emote-timejump")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if                            message.startswith("Viral All") or          message.startswith("Viralgroove All") or  message.startswith("X1"):
          if user.username == "_N_a_Y_a_":
            roomUsers = (await self.highrise.get_room_users()).content
            for roomUser, _ in roomUsers:
                await self.highrise.send_emote("dance-tiktok9", roomUser.id)
                
        if message.startswith("W1"):
          if user.username == "_N_a_Y_a_":
            roomUsers = (await self.highrise.get_room_users()).content
            for roomUser, _ in roomUsers:
